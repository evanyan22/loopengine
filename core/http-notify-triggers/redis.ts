// A DurableApprover backed by a Redis list instead of a notification —
// the "pushing to a queue" option AgentConfig.httpNotifier's `channel:
// 'redis'` constructs. Uses `ioredis`, already a real dependency here
// (core/session-store.ts's own RedisSessionStore), so no new dependency
// to add. Approval-only, deliberately — see ./database.ts's own header
// comment for why this isn't a "notification channel" with a
// question-handler/lifecycle-sender sibling to merge in.
//
...
export interface RedisQueueApproverOptions {
  redis: Redis
...
export class RedisQueueApprover implements DurableApprover {
  private readonly redis: Redis
  private readonly queueKey: string

  constructor(options: RedisQueueApproverOptions) {
  ...
  requestDurableApproval(
    ...
    this.redis.rpush(this.queueKey, JSON.stringify(entry)).catch((err) => {
      console.error(`[loopengine] RedisQueueApprover: rpush failed for pendingId '${pendingId}':`, err)
    })
